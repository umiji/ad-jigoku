...
- ゲーム / LP ルートに実広告を入れる（DECISIONS_v0.2 §8.1, ADR-012, SAFE-13）
- サードパーティの広告ブロッカーを推奨する（DECISIONS_v0.2 §6.4。案内するのはブラウザ標準機能のみ）
...
| Simulator | (v0.1 用語。廃止。`Shell` + `Behavior` に置き換え。ADR-009) |
